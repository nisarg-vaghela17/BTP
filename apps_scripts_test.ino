...
void setup() {
  ...
  Serial.println("\n=== ESP32 Sensor Logger (Manual MAX6675) ===");
...
void loop() {
  ...
  if (currentTime - lastReadTime >= readInterval) {
    ...
    if (currentTime - lastSendTime >= sendInterval) {
      sendToGoogleSheets(thermoTemp, adcVal, Vout, Rsens);
...
float readMAX6675() {
  ...
  Serial.print("  [DEBUG] Raw data: 0x");
  ...
  if (data & 0x0004) {
    Serial.println("  [DEBUG] Bit D2 set - Thermocouple open!");
    return NAN;
  }
